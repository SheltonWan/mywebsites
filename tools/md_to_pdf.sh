#!/usr/bin/env bash
# tools/md_to_pdf.sh — Markdown → PDF 一键转换流水线
#
# 用法：
#   bash tools/md_to_pdf.sh docs/backend/report.md
#   bash tools/md_to_pdf.sh docs/frontend/*.md     # 批量
#
...
if [[ $# -eq 0 ]]; then
    echo "Usage: bash tools/md_to_pdf.sh <docs/[subdir/]file.md> [...]" >&2
    exit 1
fi
...
for MD_FILE in "$@"; do
    ...
    echo "  md   → $DOCX"
    python3 "$WORKSPACE/tools/md2word.py" "$ABS_MD" "$DOCX"

    echo "  docx → $PDF"
    python3 "$WORKSPACE/tools/docx2pdf.py" "$DOCX" "$PDF"
